/* 
 * a10-tp.c - A10 SoC Resistive Touch Panel (RTP) embedded sensor driver only 
 * ------------
 * modified by Zhang Tong <lovewilliam@gmail.com>
 * calibrated using a ds18b20 mounted on the chip surface
...
#define TP_TPR_TEMP_PERIOD        1953
...
{ 
	...
	int temp_data = atomic_read(&data->temperature);
        if (attr->index == 1) 
                return sprintf(buf, "2613026\n"); 
        if (attr->index == 2) 
                return sprintf(buf, "-2632669\n"); 
        if (attr->index == 3) 
                return sprintf(buf, "a10_tp\n"); 
        if (temp_data == TP_TEMP_NODATA) 
                return -EAGAIN; 
	//modified by Zhang Tong
	return sprintf(buf, "%d\n", (-2632669+temp_data*1281));
} 
...
static void platform_a10_tp_release(struct device * dev)
{
	    return ;
}
...
{ 
        ...
        if (!(reg_val & (TP_DOWN_PENDING | FIFO_DATA_PENDING | TP_UP_PENDING))) { 
                if (reg_val & TP_TEMP_IRQ_PENDING) { 
                        ...
                        atomic_set(&ts_data->temperature, reg_val);
                        return IRQ_HANDLED; 
                } 
                return IRQ_NONE; 
        } 
...
static int __devinit a10_tp_hwmon_probe(struct platform_device *pdev) 
{ 
        int err = 0; 
        ...
        atomic_set(&ts_data->temperature , TP_TEMP_NODATA);
...
static int __devexit a10_tp_hwmon_remove(struct platform_device *pdev) 
{ 
...
static struct platform_driver a10_tp_hwmon_driver = { 
        .probe                = a10_tp_hwmon_probe, 
        .remove                = __devexit_p(a10_tp_hwmon_remove), 
        .driver                = { 
                .name        = "a10-tp-hwmon", 
        }, 
}; 
 
static struct resource a10_tp_hwmon_resource[] = { 
        { 
...
struct platform_device a10_tp_hwmon_device = { 
        .name                = "a10-tp-hwmon", 
        .id                  = -1, 
        .resource            = a10_tp_hwmon_resource, 
        .num_resources       = ARRAY_SIZE(a10_tp_hwmon_resource),
	.dev = {
		.release	= platform_a10_tp_release,
	},
}; 
 
static int __init a10_tp_hwmon_init(void) 
{ 
        platform_device_register(&a10_tp_hwmon_device); 
        return platform_driver_register(&a10_tp_hwmon_driver); 
} 
 
static void __exit a10_tp_hwmon_exit(void) 
{ 
        platform_driver_unregister(&a10_tp_hwmon_driver); 
        platform_device_unregister(&a10_tp_hwmon_device); 
} 
 
module_init(a10_tp_hwmon_init); 
module_exit(a10_tp_hwmon_exit); 
 
MODULE_AUTHOR("Corentin LABBE");
MODULE_DESCRIPTION("A10 SoC embedded touchscreen hwmon driver");
MODULE_LICENSE("GPL");
